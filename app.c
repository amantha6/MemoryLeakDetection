...
main(int argc, char **argv){
    ...
    mld_set_dynamic_object_as_root(object_db, aditi);

    student_t *aarushi = xcalloc(object_db, "student_t", 1);
    strncpy(aarushi->stud_name, "Aarushi", strlen("Aarushi"));
    //aditi->best_colleage = aarushi;

    emp_t *adhira = xcalloc(object_db, "emp_t", 2);
    mld_set_dynamic_object_as_root(object_db, adhira);
    adhira->p = xcalloc(object_db, "int", 1);
